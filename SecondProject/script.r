...
library(ggplot2)
library(GGally)
ggpairs(data %>% na.omit(), aes(colour = fac, alpha=0.5))


# OFF_RATING e DEF_RATING
ggplot(data, aes(OFF_RATING, DEF_RATING, col=PLAYOFF)) + geom_jitter() + 
    scale_color_discrete(labels = c("Not Qualified", "Qualified"))

library(klaR)
partimat(PLAYOFF ~ OFF_RATING + DEF_RATING + PTS + TOV, data=data, method="lda")
...
#plot(data.qda.p$x, pch=20, col = data$PLAYOFF + 2, main = "Classificazione tramite Analisi Driscriminante Lineare")
legend("bottomright",
       inset = 0.02,
       legend = c("Non Qualificate ai Playoff", "Qualificate ai Playoff"),
       col = c("red", "green3"),
...
boxplot(acc, main="Autovalutazione", xlab="Classification Methods", ylab="Accuracy", xaxt="n")
...
lines(accLdaMedia, col = "green", lty=2)
...
# convertire la classe in factor
...
# qda
partimat(PLAYOFF ~ OFF_RATING + DEF_RATING + PTS + TOV, data=data, method="qda")
